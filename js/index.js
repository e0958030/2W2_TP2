...
let audio = {
    ...
    musiqueQuiz: new Audio('sons/musiqueQuiz.mp3') //Musique prise du jeu "The Sims" par Maxis
};
...
let curseurPerso = null;

//Appeler la fonction
gestionCurseurPerso();

//Fonction qui va gérer le curseur et la gestion de la souris
function gestionCurseurPerso(){

    //Récupérer la balise avec le curseur
    curseurPerso = document.querySelector(".curseurPerso");

    //Pour gérer le curseur s'il y a un survol de la souris
    if (window.matchMedia("(hover:hover)").matches){
        document.querySelector("body").style.cursor = "none";

        //Rajouter un écouteur d'événement
        document.addEventListener("mousemove", deplacerCurseur, true);
    }
     else{
        document.querySelector("body").style.cursor = "auto";
        curseurPerso.style.display = "none";
    }
}

//Fonction pour déplacer le curseur en soi
function deplacerCurseur(){
    
    //Récupérer la racine du document
    let racineRoot = document.querySelector(":root");

    //Changer la position en X et en Y du curseur
    racineRoot.style.setProperty("--mouse-x", event.clientX + "px")
    racineRoot.style.setProperty("--mouse-y", event.clientY + "px")
}

//Fonction pour modifier le curseur selon son état et lui ajouter une classe sinon
function modifierCurseur(event){
    if(event.type == "mouseover"){
        curseurPerso.classList.add("survolCurseur");
    }else{
        curseurPerso.classList.remove("survolCurseur");
    }
}
...
function debuterQuiz(){
    ...
    //Jouer la musique de fond du jeu
    audio.musiqueQuiz.play();
